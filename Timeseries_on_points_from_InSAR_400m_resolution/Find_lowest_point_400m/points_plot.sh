...
for file in "${source_dir}"/LOWESS_point_*_ts.txt; do
  # Extract the well number from the file name
    point_number=$(echo "$file" | sed -n 's/.*LOWESS_point_\([0-9]*\)\_ts.txt/\1/p')
    # echo $point_number
  # Define the output file name for the plot
    ...
    value5=$(awk -v point="Point_$point_number" '$1 == point { print $5 }' $result_file)
    value6=$(awk -v point="Point_$point_number" '$1 == point { print $6 }' $result_file)
